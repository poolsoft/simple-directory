<?php
/**Allow image upload for category description  
 * Based on: Categories Images by Muhammad Said El Zahlan (http://zahlan.net/)
 
 */
?>
<?php
if (!defined('DIR_PLUGIN_URL'))
	define('DIR_PLUGIN_URL', untrailingslashit(plugins_url('', __FILE__)));

define('DIR_IMAGE_PLACEHOLDER', DIR_PLUGIN_URL."/images/placeholder.png");

// l10n
load_plugin_textdomain('sdir', FALSE, 'simple-directory/languages');

add_action('admin_init', 'sdir_categoryimages_init');
function sdir_categoryimages_init() {
	$sdir_taxonomies = get_taxonomies();
	if (is_array($sdir_taxonomies)) {
		$sdir_options = get_option('sdir_options');
		if (empty($sdir_options['excluded_taxonomies']))
			$sdir_options['excluded_taxonomies'] = array();
		
	    foreach ($sdir_taxonomies as $sdir_taxonomy) {
			if (in_array($sdir_taxonomy, $sdir_options['excluded_taxonomies']))
				continue;
	        add_action($sdir_taxonomy.'_add_form_fields', 'sdir_add_texonomy_field');
			add_action($sdir_taxonomy.'_edit_form_fields', 'sdir_edit_texonomy_field');
			add_filter( 'manage_edit-' . $sdir_taxonomy . '_columns', 'sdir_taxonomy_columns' );
			add_filter( 'manage_' . $sdir_taxonomy . '_custom_column', 'sdir_taxonomy_column', 10, 3 );
	    }
	}
}

function sdir_add_style() {
...
function sdir_add_texonomy_field() {
...
function sdir_edit_texonomy_field($taxonomy) {
	if (get_bloginfo('version') >= 3.5)
		wp_enqueue_media();
	else {
		wp_enqueue_style('thickbox');
	...
	if (sdir_taxonomy_image_url( $taxonomy->term_id, NULL, TRUE ) == Z_IMAGE_PLACEHOLDER) 
		$image_text = "";
	else
		$image_text = sdir_taxonomy_image_url( $taxonomy->term_id, NULL, TRUE );
